...
// REV_RREQ
//-----------------------------------------------------------------------------
RrevreqHeader::RrevreqHeader(uint8_t flags,
                       uint8_t reserved,
                       uint8_t hopCount,
                       uint32_t requestID,
                       Ipv4Address dst,
...
NS_OBJECT_ENSURE_REGISTERED(RrevreqHeader);

TypeId
RrevreqHeader::GetTypeId()
{
    static TypeId tid = TypeId("ns3::aodv::RrevreqHeader")
                            .SetParent<Header>()
                            .SetGroupName("Aodv")
                            .AddConstructor<RrevreqHeader>();
    return tid;
}

TypeId
RrevreqHeader::GetInstanceTypeId() const
{
    return GetTypeId();
}

uint32_t
RrevreqHeader::GetSerializedSize() const
{
    return 23;
}

void
RrevreqHeader::Serialize(Buffer::Iterator i) const
{
    i.WriteU8(m_flags);
    i.WriteU8(m_reserved);
    i.WriteU8(m_hopCount);
...
uint32_t
RrevreqHeader::Deserialize(Buffer::Iterator start)
{
    Buffer::Iterator i = start;
    m_flags = i.ReadU8();
    m_reserved = i.ReadU8();
...
void
RrevreqHeader::Print(std::ostream& os) const
{
    os << "RREQ ID " << m_requestID << " destination: ipv4 " << m_dst << " sequence number "
       << m_dstSeqNo << " source: ipv4 " << m_origin << " sequence number " << m_originSeqNo
       << " flags:"
...
std::ostream&
operator<<(std::ostream& os, const RrevreqHeader& h)
{
    h.Print(os);
    return os;
}
...
void
RrevreqHeader::SetGratuitousRrep(bool f)
{
    if (f)
    {
        m_flags |= (1 << 5);
...
bool
RrevreqHeader::GetGratuitousRrep() const
{
    return (m_flags & (1 << 5));
}

void
RrevreqHeader::SetDestinationOnly(bool f)
{
    if (f)
    {
        m_flags |= (1 << 4);
...
bool
RrevreqHeader::GetDestinationOnly() const
{
    return (m_flags & (1 << 4));
}

void
RrevreqHeader::SetUnknownSeqno(bool f)
{
    if (f)
    {
        m_flags |= (1 << 3);
...
bool
RrevreqHeader::GetUnknownSeqno() const
{
    return (m_flags & (1 << 3));
}

bool
RrevreqHeader::operator==(const RrevreqHeader& o) const
{
    return (m_flags == o.m_flags && m_reserved == o.m_reserved && m_hopCount == o.m_hopCount &&
            m_requestID == o.m_requestID && m_dst == o.m_dst && m_dstSeqNo == o.m_dstSeqNo &&
            m_origin == o.m_origin && m_originSeqNo == o.m_originSeqNo);
